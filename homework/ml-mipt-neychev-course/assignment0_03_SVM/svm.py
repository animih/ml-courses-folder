...
def rbf(x_1, x_2, sigma=1.):
    ...
    '''
    distances = (torch.sum(x_2**2, axis =1)+(torch.sum(x_1**2, axis =1))[:, np.newaxis] -
        2*torch.matmul(x_1, x_2.T))
    return torch.exp(-distances/(2*sigma**2)).type(torch.float32)
...
def hinge_loss(scores, labels):
    ...
    tmp = (1-labels*scores)
    return torch.sum(tmp[tmp > 0])/scores.shape[0]
...
class SVM(BaseEstimator, ClassifierMixin):
    ...
    def fit(self, X, Y):
        ...
        for epoch in range(self.epochs):
            ...
            for i in range(0, n_obj, self.batch_size):
                ...
                preds = torch.matmul(k_batch, self.betas)+self.bias*torch.ones(self.batch_size, 1)### YOUR CODE HERE # get the matrix product using SVM parameters: self.betas and self.bias
                preds = preds.flatten()
    ...
    def predict_scores(self, batch):
        with torch.no_grad():
            ...
            ret = torch.matmul(K, self.betas)+self.bias*torch.ones(batch.shape[0], 1)
            return ret.flatten() ### YOUR CODE HERE
